...
(function() {
  var checksumCalculator, crypto, fs, orphanKiller, path, request, slugify, thumbnail, url, util, _,
    __slice = [].slice,
  ...
  exports.killAllOrphans = function(s) {
    return s.replace(orphanKiller, "$1&nbsp;$2");
  };
  ...
  thumbnail = function(file, width, height) {
    ...
    path.exists(thumbnailPath, function(exists) {
      if (!exists) {
        return read(convert);
      }
    });
