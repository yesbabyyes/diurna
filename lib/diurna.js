(function() {
  var RESERVED_NAMES, buildPage, buildPages, buildScripts, buildStyles, eco, formatDate, fs, getLayout, link, log, markdown, mkdirs, path, read, slugify, stitch, stylus, util, write, _, _verbosity;
  ...
  _verbosity = 0;
  RESERVED_NAMES = ["styles", "scripts", "images"];
  ...
  buildPages = function(from, to) {
    ...
    traverse = function(options, parent) {
      ...
      return fs.readdir(currentDir, function(err, files) {
        var basename, context, dirNames, extension, file, filePath, layouts, node, page, pageLayout, pages, stat, _i, _j, _len, _len2, _ref2, _ref3, _results;
        if (err) {
        ...
        for (page in pages) {
          ...
          layouts = parent.layouts ? [].concat(parent.layouts) : [];
  ...
  buildStyles = function(from, to) {
    return fs.readFile(from, "utf8", function(err, str) {
      ...
      return stylus(str).set("filename", from).use(require("nib")())["import"]("nib").render(function(err, css) {
        if (err) {
          return util.error(err);
        }
